...
const Home = () => {
  ...
  const [showTeachers,setTeachers]=useState(0);
  // console.log(user);
  useEffect(() => {
  ...
  for (let i = 0; i < myUser?.fixed?.length; i++) {
    ...
    if (day === 'Monday') {
      if (timeslot === '09-10AM') {
        ...
        console.log('changedddddddddddddddddddddddddd');
  ...
  for (let i = 0; i < which.size; i++) {
    for (let j = 0; j <= 6; j++) {
      if (monday[i][j] === 'null') {
        ...
        console.log(monday[i][j],'asdfggfdsdfgfdsdfgvg');
  ...
  let myTeacherList=[];
  teachers.forEach((values, key) => {
    myTeacherList.push(key);
})
console.log(myTeacherList);
  ...
  return (
    <div id='content'>
      ...
      }
      {
        showTeachers===1 &&
        <div>
                {myTeacherList.map(function(data) {
                    return (
                      <Teacher teacher={data} mon={monday} tue={tue} wed={wed} thurs={thurs} fri={fri} which={which} x={which.size}/>
                    )
          })}
        </div>
      }
